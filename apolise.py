...
CHANNEL_ID = os.environ.get('CHANNEL_ID', '@apolise')
...
async def bg_register_user(user_id: int):
    ...
    try:
        # Daftar/Update di tabel usersi
        await db(lambda: supabase.table("usersi").upsert({"user_id": user_id}).execute())
...
async def cmd_start_inbox(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton("🧭 Navigasi Menu", url="https://t.me/apolise/1640")], 
        [InlineKeyboardButton("💬 Testimoni", url="https://t.me/testlucs")], 
        [InlineKeyboardButton("📊 Result", url="https://t.me/saumple"), InlineKeyboardButton("⭐ Honest Review", url="https://t.me/mirezae")]
    ]
    await update.message.reply_text(
        f"Halo <b>{html.escape(user.first_name)}</b>! Selamat datang di bot pemesanan <b>@APOLISE</b> 🛒\n\nAda yang bisa kami bantu? Silakan langsung ketik pesan di sini ya!", 
...
async def get_all_user_ids():
    ...
    offset = 0
    while True:
        res = await db(lambda o=offset: supabase.table("usersi").select("user_id").range(o, o + page_size - 1).execute())
...
async def handle_broadcast_delete_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    sql_query = f"-- Jalankan di SQL Editor Supabase\nDELETE FROM usersi WHERE user_id IN ({', '.join(str(uid) for uid in to_delete)});"
...
async def cmd_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    res = await db(lambda: supabase.table("loyalty_stats").select("*").eq("user_id", target_user_id).execute())
    stats = res.data[0] if res.data else {"teleprem_spent": 0, "stars_spent": 0, "profneeds_spent": 0}
...
async def cmd_addreferal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        pesan_pengajak = (
            ...
            f"Thanks ya udah ngajakin temen kamu jajan Teleprem di @APOLISE. "
...
def main():
    ...
    logger.info("✅ APOLISE Bot V2 (Background Async + Full Features + Anti-Freeze) siap berjalan!")
